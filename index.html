...
<head>
    ...
    <title>Gandra - Acupuntura & Saúde Integrativa</title>
...
<body class="bg-gray-50 text-gray-800">
    ...
    <header class="bg-white">
        <div class="container mx-auto px-6 py-12 text-center">
            ...
            <p class="mt-2 text-lg text-emerald-700">Acupuntura &amp; Saúde Integrativa</p>
            <p class="mt-4 max-w-2xl mx-auto text-gray-600">
                Encontre o equilíbrio e renove suas energias. Oferecemos um cuidado completo e personalizado para o seu bem-estar.
            </p>
    ...
    <footer class="text-center p-8 bg-white mt-16">
        <p class="text-gray-500 text-sm">&copy; 2025 Gandra - Todos os direitos reservados.</p>
    ...
    <script type="text/babel">
        ...
        const SparkleIcon = ({ className }) => (
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
                ...
                <path d="M20 5L20.5 7L22 7.5L20.5 8L20 9L19.5 8L18 7.5L19.5 7L20 5Z" />
                <path d="M5 20L4.5 18L3 17.5L4.5 17L5 15L5.5 17L7 17.5L5.5 18L5 20Z" />
            </svg>
        );

        // --- FUNÇÃO DE CHAMADA DA API GEMINI ---
        async function generateGeminiText(prompt) {
            const apiKey = ""; // Deixar em branco, a plataforma injetará a chave.
        ...
        const serviceListForPrompt = services.map(s => s.name).join(", ");
        ...
        function App() {
          ...
          const [suggestedServiceId, setSuggestedServiceId] = useState(null);
          ...
          const handleSymptomSuggestion = async () => {
              ...
              const prompt = `Você é um assistente de uma clínica de saúde integrativa. Baseado nos sintomas a seguir, qual destes tratamentos você recomendaria? Tratamentos disponíveis: ${serviceListForPrompt}. Sintomas: "${symptomQuery}". Responda apenas com o nome exato de um dos tratamentos da lista.`;
              
              const suggestionText = await generateGeminiText(prompt);
              
              const foundService = services.find(service => suggestionText.toLowerCase().includes(service.name.toLowerCase()));
              
              if (foundService) {
                  setSuggestedServiceId(foundService.id);
              } else {
                  console.log("Nenhum serviço correspondente encontrado para:", suggestionText);
              }
          ...
          return (
            <div className="w-full max-w-4xl bg-white rounded-2xl shadow-lg flex flex-col md:flex-row overflow-hidden">
              ...
              <div className="w-full md:w-1/3 bg-emerald-700 text-white p-8 flex flex-col">
                ...
                <p className="text-emerald-200 text-sm mb-8">Acupuntura & Saúde Integrativa</p>
              ...
              <div className="w-full md:w-2/3 p-8 md:p-12">
                {step === 1 && <ServiceSelector services={services} onSelect={handleServiceSelect} onSymptomChange={setSymptomQuery} symptomQuery={symptomQuery} onSymptomSubmit={handleSymptomSuggestion} isSuggesting={isSuggesting} suggestedServiceId={suggestedServiceId}/>}
        ...
        const ServiceSelector = ({ services, onSelect, onSymptomChange, symptomQuery, onSymptomSubmit, isSuggesting, suggestedServiceId }) => (
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Escolha um serviço</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {services.map(service => (
                <button key={service.id} onClick={() => onSelect(service)} className={`text-left p-4 border rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 ${suggestedServiceId === service.id ? 'bg-emerald-100 border-emerald-500 ring-2 ring-emerald-500 animate-pulse' : 'border-gray-200 hover:border-emerald-500 hover:bg-emerald-50'}`}>
            ...
            <div className="mt-10 pt-6 border-t">
                <h3 className="text-xl font-bold text-gray-800 mb-3 flex items-center"><SparkleIcon className="w-6 h-6 mr-2 text-emerald-500" />Não sabe qual escolher?</h3>
                <p className="text-gray-600 mb-4">Descreva como você está se sentindo e nós sugerimos o melhor tratamento para você.</p>
                <textarea 
                    value={symptomQuery}
                    onChange={(e) => onSymptomChange(e.target.value)}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 mb-4"
                    rows="3"
                    placeholder="Ex: Estou com dor nas costas e muito estresse no trabalho..."
                />
                <button 
                    onClick={onSymptomSubmit}
        ...
        const DateTimePicker = ({ selectedDate, onDateSelect, onTimeSelect, availableTimes, selectedTime, onBack, bookedAppointments, selectedService }) => {
          ...
          return (
            <div>
              ...
              <div className="mt-6">
                <h3 className="font-semibold text-gray-700 mb-4 flex items-center"><ClockIcon className="w-5 h-5 mr-2 text-gray-500"/> Horários disponíveis</h3>
        ...
        const ConfirmationModal = ({ service, date, time, user, onClose }) => {
            ...
            const fetchPrepTips = async () => {
                ...
                const prompt = `Você é um assistente de uma clínica de saúde integrativa e acupuntura. Um paciente agendou uma "${service.name}". Forneça 3 dicas curtas e úteis, em formato de lista, para que o paciente possa se preparar para a sessão. Fale de forma amigável e calmante.`;
                const tips = await generateGeminiText(prompt);
                setPrepTips(tips);
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 text-center relative transform transition-all animate-fade-in-up">
                    ...
                    <h3 className="text-2xl font-bold text-gray-900">Agendamento Confirmado!</h3>
                    <p className="text-gray-600 mt-2 mb-6">Sua consulta foi marcada com sucesso. Um e-mail de confirmação será enviado.</p>
                    ...
                    {!prepTips && !isLoadingTips && (
                        <button onClick={fetchPrepTips} className="w-full border border-emerald-600 text-emerald-600 font-bold py-3 px-4 rounded-lg hover:bg-emerald-50 transition-all duration-300 flex items-center justify-center">
                            ✨ Ver Dicas de Preparação
                        </button>
                    )}

                    {isLoadingTips && (
                         <div className="w-full text-emerald-600 font-bold py-3 px-4 rounded-lg flex items-center justify-center">
                    ...
                    {prepTips && (
                        <div className="text-left bg-emerald-50 p-4 rounded-lg space-y-2 text-sm">
                            <h4 className="font-bold mb-2">Dicas para sua sessão:</h4>
                            <div className="whitespace-pre-wrap text-gray-700">{prepTips}</div>
                        </div>
                    )}
                    
                    <button onClick={onClose} className="mt-8 w-full bg-emerald-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-emerald-700 transition-colors">
                        Fechar
                    </button>
                </div>
            </div>
